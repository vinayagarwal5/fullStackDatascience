...
set_background(Path(__file__).with_name("5.png"))
...
def chatbot_response(user_input):
    ...
    similarities = {
        question: torch.nn.functional.cosine_similarity(
            torch.from_numpy(user_embedding),
            torch.from_numpy(predefined_embeddings[question]),
        ).item()
